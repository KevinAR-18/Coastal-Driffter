...
// Serial1: Terhubung ke simpleRTK2B Base (RTCM3 Output)
// Serial2: Terhubung ke Radio Telemetry / LoRa Transceiver (RX2=17, TX2=16 di Arduino Mega)
// Serial: USB Serial ke Laptop/PC (Format Output: JSON Stream 115200 bps)
...
void processRoverPacket(const TelemetryPacket& pkt) {
  // Format data menjadi JSON String dan kirim via USB Serial ke PC Laptop Dashboard
  ...
  while (RADIO_SERIAL.available()) {
    ...
    if (rxIndex >= sizeof(TelemetryPacket)) {
      ...
      if (calcCrc == inPacket.checksum) {
        processRoverPacket(inPacket); // Valid Packet! Stream JSON to PC Dashboard
      }
      rxIndex = 0;
    }
  }
